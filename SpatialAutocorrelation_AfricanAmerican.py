...
# # Spatial Autocorrelation: African American
# 
# African American = percentage of population that is non-Hispanic Black or African American
...
# predictor variable
dfmw = df.dropna(subset=['percent Non-Hispanic African American2017  raw value'])
dfmw.head(2)
...
db = dfmw.set_index("percent Non-Hispanic African American2017  raw value", drop=False)
db.crs
...
y = db2['percent Non-Hispanic African American2017  raw value']
...
# `percent Non-Hispanic African American2017  raw value` has a moderate/modest positive spatial autocorrelation (.77). This means that there is a clustered pattern as opposed to a random or dispersed pattern. The p-value is .001, so we reject the null - the pattern reached significance. 
...
db2['lag'] = weights.spatial_lag.lag_spatial(w, db2['percent Non-Hispanic African American2017  raw value'])
...
sbn.regplot(
    x="percent Non-Hispanic African American2017  raw value",
    y="lag",
    ci=None,
    data=db2,
    line_kws={"color": "r"},
...
db2.plot(column='percent Non-Hispanic African American2017  raw value', 
         cmap='Blues', 
         edgecolor='white',
         linewidth=0.2,
         alpha=0.8,
...
ax.set_axis_off()
ax.set_title('Local Statistics: African American'
    )
...
ax.set_axis_off()
ax.set_title(
    'Moran Cluster Map: African American Significant Hot Spots and Cold Spots'
    )
