...
# # Spatial Autocorrelation: Low Birthweight
...
# target variable
dfmw = df.dropna(subset=['% Low Birthweight 20142020'])
dfmw.head(2)
...
# Index table on variable of interest "% Low Birthweight 20142020"
db = dfmw.set_index("% Low Birthweight 20142020", drop=False)
db.crs
...
y = db2['% Low Birthweight 20142020']
...
# `% Low Birthweight 20142020`has a high positive spatial autocorrelation (.79). This means that there is a clustered pattern as opposed to a random or dispersed pattern. The p-value is .001, so we reject the null - the pattern reached significance. 
...
db2['lbw_rate_lag'] = weights.spatial_lag.lag_spatial(w, db2['% Low Birthweight 20142020'])
...
# figures without light gray count base

# Set up figure and axes
f, axs = plt.subplots(nrows=2, ncols=1, figsize=(18, 15))
...
db2.plot(column='% Low Birthweight 20142020', 
         cmap='Blues', 
         edgecolor='white',
         linewidth=0.2,
         alpha=0.8,
...
for i, ax in enumerate(axs.flatten()):
    ...
    ax.set_title(
    [
    'Local Statistics: Rate of Low Birth Weight',
    'Moran Cluster Map: LBW Significant Hot Spots and Cold Spots'
    ][i], y=0
    )
...
#selecting data 
lbw_db2_e = db2_e[['FIPS','CountyName','STATE_NAME','StateAbbreviation','% Low Birthweight 20142020','labels']]

# renaming columns
lbw_db2_e.columns = ['FIPS','CountyName','State','StateAbbreviation','LBW','LBWClusters']
...
# lbw_db2_e.to_csv('lbw_cluster.csv',index=False)


# Conclusion: Dependent variable, low birthweight rate, does vary spatially. 
