...
# # Spatial Autocorrelation: Preventable Hospital Stays
# 
...
# predictor variable
dfmw = df.dropna(subset=['Preventable hospital stays 2017 raw value'])
dfmw.head(2)
...
db = dfmw.set_index("Preventable hospital stays 2017 raw value", drop=False)
db.crs
...
y = db2['Preventable hospital stays 2017 raw value']
...
# `Preventable hospital stays 2017 raw value` has a moderate/modest positive spatial autocorrelation (.74). This means that there is a clustered pattern as opposed to a random or dispersed pattern. The p-value is .001, so we reject the null - the pattern reached significance. 
...
db2['lag'] = weights.spatial_lag.lag_spatial(w, db2['Preventable hospital stays 2017 raw value'])
...
sbn.regplot(
    x="Preventable hospital stays 2017 raw value",
    y="lag",
    ci=None,
    data=db2,
    line_kws={"color": "r"},
...
db2.plot(column='Preventable hospital stays 2017 raw value', 
         cmap='copper_r', 
         edgecolor='white',
         linewidth=0.2,
         alpha=0.8,
...
ax.set_axis_off()
ax.set_title('Local Statistics: Preventable Hospital Stays'
    )
...
ax.set_axis_off()
ax.set_title(
    'Moran Cluster Map: Preventable Hospital Stays Significant Hot Spots and Cold Spots'
    )
