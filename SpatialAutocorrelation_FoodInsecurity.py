...
# # Spatial Autocorrelation: Food Insecurity 
# 
...
# predictor variable
dfmw = df.dropna(subset=['Food insecurity 2017 raw value'])
dfmw.head(2)
...
db = dfmw.set_index("Food insecurity 2017 raw value", drop=False)
db.crs
...
y = db2['Food insecurity 2017 raw value']
...
# `Food insecurity 2017 raw value` has a moderate/modest positive spatial autocorrelation (.90). This means that there is a clustered pattern as opposed to a random or dispersed pattern. The p-value is .001, so we reject the null - the pattern reached significance. 
...
db2['lag'] = weights.spatial_lag.lag_spatial(w, db2['Food insecurity 2017 raw value'])
...
sbn.regplot(
    x="Food insecurity 2017 raw value",
    y="lag",
    ci=None,
    data=db2,
    line_kws={"color": "r"},
...
db2.plot(column='Food insecurity 2017 raw value', 
         cmap='BrBG', 
         edgecolor='white',
         linewidth=0.2,
         alpha=0.8,
...
ax.set_axis_off()
ax.set_title('Local Statistics: Food Insecurity'
    )
...
ax.set_axis_off()
ax.set_title(
    'Moran Cluster Map: Food Insecurity Significant Hot Spots and Cold Spots'
    )
