...
# # Spatial Autocorrelation: Percent Rural
# 
# Percent Rurual = Percentage of population living in a rural area
...
# predictor variable
dfmw = df.dropna(subset=['percent Rural 2010 raw value'])
dfmw.head(2)
...
db = dfmw.set_index("percent Rural 2010 raw value", drop=False)
db.crs
...
y = db2['percent Rural 2010 raw value']
...
# `percent Rural 2010 raw value` has a moderate/modest positive spatial autocorrelation (.56). This means that there is a clustered pattern as opposed to a random or dispersed pattern. The p-value is .001, so we reject the null - the pattern reached significance. 
...
db2['lag'] = weights.spatial_lag.lag_spatial(w, db2['percent Rural 2010 raw value'])
...
sbn.regplot(
    x="percent Rural 2010 raw value",
    y="lag",
    ci=None,
    data=db2,
    line_kws={"color": "r"},
...
db2.plot(column='percent Rural 2010 raw value', 
         cmap='Blues', 
         edgecolor='white',
         linewidth=0.2,
         alpha=0.8,
...
ax.set_axis_off()
ax.set_title('Local Statistics: Percent Rural'
    )
# Tight layout to minimise in-betwee white space
f.tight_layout()

#Save figure
#plt.savefig("mw_rural")
...
ax.set_axis_off()
ax.set_title(
    'Moran Cluster Map: Percent Rural Significant Hot Spots and Cold Spots'
    )
